...
@section('title' , __('website.product'))
    ...
    <section class="product product-info">
        <div class="container">
            <div class="blog-bradcrum">
                ...
                <span class="devider">/</span>
                <span><a href="#">{{ $product->getTranslation('name',app()->getLocale()) }}</a></span>
            </div>
                ...
                <div class="row ">
                        ...
                        <div class="product-info-img">
                            <div class="swiper product-top">
                                @if($product->has_discount)
                                    @livewire('website.product.product-discount-percentage', ['product' => $product])
                                @endif
                            ...
                            <div class="swiper product-bottom">
                                <div class="swiper-wrapper">
                                    @foreach($product->images as $imageBottom)
                                        <div class="swiper-slide slider-bottom-img">
                                            <img src="{{ asset($imageBottom->name) }}"
                                                 alt="{{ $product->getTranslation('name',app()->getLocale()) }}">
                                        </div>
                                    @endforeach
                                </div>
                    ...
                    <div class="col-md-6">
                        <div class="product-info-content">
                            <span class="wrapper-subtitle">{{ $product->category->getTranslation('name',app()->getLocale()) }}</span>
                            <h5>
                            ...
                            <div class="ratings">
                                ...
                                <span class="text">{{ $product->reviews->count() }} {{ __('website.reviews_count') }}</span>
                            </div>


                            @livewire('website.product.product-show', ['product' => $product])

                            <div class="product-quantity">
                                <div class="quantity-wrapper">

                                    {{--   wish list icon   --}}
                                    @livewire('website.wish-list.product-wish-list-icon', ['productId' => $product->id])
                                    {{--   add to cart section   --}}

                                    @livewire('website.cart.add-to-cart-product-section', ['product' => $product])
                            ...
                            <div class="product-details">
                                ...
                                @if($product->brand)
                                    <p class="category">{{ __('website.brand') }} : <span
                                                class="inner-text">{{ $product->brand->getTranslation('name',app()->getLocale()) }}</span>
                                    </p>
                                @endif
                                {{--        <p class="tags">Tags : <span--}}
                                {{--                    class="inner-text">{{ $product->tags->pluck('name')->implode(', ') }}</span>--}}
                                {{--        </p>--}}
                            ...
                            <div class="product-report">
                                <a href="#" class="report" onclick="modalAction('.action')">
                                    ...
                                    <span>{{ __('website.report_this_item') }}</span>
                                </a>
                                    ...
                                    <div class="login-section account-section modal-main">
                                        <div class="review-form">
                                            <div class="review-content">
                                                <h5 class="comment-title">{{ __('website.report_products') }}</h5>
                                                <div class="close-btn">
                                            ...
                                            <div class="review-form-name address-form">
                                                <label for="reporttitle"
                                                       class="form-label">{{ __('website.enter_report_title') }}</label>
                                                <input type="text" id="reporttitle" class="form-control"
                                                       placeholder="{{ __('website.reports_headline_here') }}">
                                            </div>
                                            <div class="review-form-name address-form">
                                                <label for="reportnote"
                                                       class="form-label">{{ __('website.enter_report_note') }}</label>
                                                <textarea name="ticketmassage" id="reportnote" cols="40" rows="3"
                                                          class="form-control"
                                                          placeholder="{{ __('website.type_here') }}"></textarea>
                                            </div>
                                            <div class="login-btn text-center">
                                                <a href="#" onclick="modalAction('.action')"
                                                   class="shop-btn">{{ __('website.submit_report') }}</a>
                                            </div>
                                        </div>
                                    </div>
            ...
            <div class="product-detail-section">
                <nav>
                    <div class="nav nav-tabs nav-item" id="nav-tab" role="tablist">
                        <button class="nav-link active" id="nav-home-tab" data-bs-toggle="tab"
                                ...
                                aria-selected="true">{{ __('website.description') }}
                        </button>
                        <button class="nav-link" id="nav-review-tab" data-bs-toggle="tab" data-bs-target="#nav-review"
                                ...
                                aria-selected="false">{{ __('website.reviews') }}
                        </button>
                ...
                <div class="tab-content tab-item" id="nav-tabContent">
                    ...
                    <div class="tab-pane fade" id="nav-review" role="tabpanel" aria-labelledby="nav-review-tab"
                         tabindex="0">
                        @livewire('website.product.review',['product' => $product])
                    </div>
    ...
    <section class="product weekly-sale product-weekly footer-padding">
        <div class="container">
            <div class="section-title">
                ...
                <a href="{{ route('website.product.related.product',$product->slug) }}"
                            ...
                            <a href="{{ route('website.product.show',$product->slug) }}" style="all: unset">
                                <div class="product-wrapper">
                                    <div class="product-img">
                                        ...
                                        <path d="M12 14.4482V16.5664H12.5466H13.0933V15.3957V14.2204L15.6214 16.7486L18.1496 19.2767L18.5459 18.8759L18.9468 18.4796L16.4186 15.9514L13.8904 13.4232H15.0657H16.2364V12.8766V12.33H14.1182H12V14.4482Z"
                                              fill="#181818"/>
                                        <path d="M12 14.4482V16.5664H12.5466H13.0933V15.3957V14.2204L15.6214 16.7486L18.1496 19.2767L18.5459 18.8759L18.9468 18.4796L16.4186 15.9514L13.8904 13.4232H15.0657H16.2364V12.8766V12.33H14.1182H12V14.4482Z"
                                              fill="black" fill-opacity="0.2"/>
                                        <path d="M23.4345 12.8766V13.4232H24.6052H25.7805L23.2523 15.9514L20.7241 18.4796L21.125 18.8759L21.5213 19.2767L24.0495 16.7486L26.5776 14.2204V15.3957V16.5664H27.1243H27.6709V14.4482V12.33H25.5527H23.4345V12.8766Z"
                                              fill="#181818"/>
                                        <path d="M23.4345 12.8766V13.4232H24.6052H25.7805L23.2523 15.9514L20.7241 18.4796L21.125 18.8759L21.5213 19.2767L24.0495 16.7486L26.5776 14.2204V15.3957V16.5664H27.1243H27.6709V14.4482V12.33H25.5527H23.4345V12.8766Z"
                                              fill="black" fill-opacity="0.2"/>
                                        <path d="M15.6078 23.5905L13.0933 26.1096V24.9343V23.7636H12.5466H12V25.8818V28H14.1182H16.2364V27.4534V26.9067H15.0657H13.8904L16.4186 24.3786L18.9468 21.8504L18.5596 21.4632C18.35 21.2491 18.1633 21.076 18.1496 21.076C18.1359 21.076 16.9926 22.2103 15.6078 23.5905Z"
                                              fill="#181818"/>
                                        <path d="M15.6078 23.5905L13.0933 26.1096V24.9343V23.7636H12.5466H12V25.8818V28H14.1182H16.2364V27.4534V26.9067H15.0657H13.8904L16.4186 24.3786L18.9468 21.8504L18.5596 21.4632C18.35 21.2491 18.1633 21.076 18.1496 21.076C18.1359 21.076 16.9926 22.2103 15.6078 23.5905Z"
                                              fill="black" fill-opacity="0.2"/>
                                        <path d="M21.1113 21.4632L20.7241 21.8504L23.2523 24.3786L25.7805 26.9067H24.6052H23.4345V27.4534V28H25.5527H27.6709V25.8818V23.7636H27.1243H26.5776V24.9343V26.1096L24.0586 23.5905C22.6783 22.2103 21.535 21.076 21.5213 21.076C21.5076 21.076 21.3209 21.2491 21.1113 21.4632Z"
                                              fill="#181818"/>
                                        <path d="M21.1113 21.4632L20.7241 21.8504L23.2523 24.3786L25.7805 26.9067H24.6052H23.4345V27.4534V28H25.5527H27.6709V25.8818V23.7636H27.1243H26.5776V24.9343V26.1096L24.0586 23.5905C22.6783 22.2103 21.535 21.076 21.5213 21.076C21.5076 21.076 21.3209 21.2491 21.1113 21.4632Z"
                                              fill="black" fill-opacity="0.2"/>
                                        </svg>
                                    ...
                                    <div class="product-cart-btn">
                                        <a href="cart.html" class="product-btn">{{ __('website.add_to_cart') }}</a>
                                    </div>
                                </div>
                            </a>
                        </div>
